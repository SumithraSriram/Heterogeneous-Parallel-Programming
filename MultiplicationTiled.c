...
__global__ void matrixMultiplyShared(float *A, float *B, float *C, int numARows,
  ...
  //@@ You have to use shared memory for this MP
	...
	{
		if( (row < numCRows) && (t * 16 + threadIdx.x < numAColumns))
		   sa[ty][tx] = A[row * numAColumns + t*16 +tx];
		   
		else
		   sa[ty][tx] = 0.0;
		
		if( (col < numCColumns) && (t * 16 + threadIdx.y < numAColumns))
		   sb[ty][tx] = B[ (t*16 +ty) * numBColumns  +col];
...
int main(int argc, char **argv) {
  ...
  //@@ Initialize the grid and block dimensions here
	...
	dim3 grid ( (numCColumns-1)/16+1 , (numCRows-1)/16+1 , 1);
	dim3 block (16, 16, 1);
